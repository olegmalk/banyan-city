#!/usr/bin/env python3
"""Derive `ep2-b08-nogoblin-0820` -- stop naming a goblin at a region that is
entirely guard.

TWO SAMPLES, TWO GOBLINS. `ep2-b08-fistcopy-0820` (18408 px mask) drew a whole
second goblin head. `ep2-b08-eraseonly-0820` (10020 px, 102x118 -- 1.8x smaller
by area) drew a GREEN GOBLIN FIST in place of the guard's hand. Shrinking the
region by 45% changed the SIZE of the noun and not the noun.

I PRE-COMMITTED THE NEXT STEP AS STRENGTH 0.99 -> 0.70 AND I AM NOT TAKING IT,
because the second sample distinguished something the first could not. The fail
mode I registered said "if a face, a hand or a bunched garment object appears".
What actually appeared, twice, was specifically **the goblin** -- and the prompt
names one twice:

    "...other arm pointing at the small goblin man's belly, GREEN SKIN, plump,
     adult..."

That is beat 08's whole-frame txt2img prompt, inherited byte-identical through
two derivations on the deliberate ground that unchanged wording keeps the mask
the only variable. It was the right discipline for testing the mask and it is
the wrong prompt for this pass. **The mask lies entirely on the GUARD** -- his
harness strap, his cream sleeve, his brown cuff. There is no goblin in it and
there is no hand in it. Handing that region a prompt that names a green-skinned
goblin, at strength 0.99, and then being surprised by a green fist is the error.

So this rung feeds the pass a prompt describing WHAT IS ACTUALLY IN THE REGION,
and puts the two nouns both samples produced into the negative. One variable:
the prompt. The init, the mask, the strength, the steps and the cfg are all
byte-identical to the parent and are not overridden at all.

IF THIS FAILS TOO, the pre-committed step is strength 0.99 -> 0.70, and after
that the honest conclusion is that a tool with no spatial conditioning cannot be
asked to erase a limb, and beat 08's grip goes back to the txt2img route.
...
JOB = "ep2-b08-nogoblin-0820"
SEED = 20260822

INIT = "08-first-citizen-eraseonly-0820.png"  # UNCHANGED from the parent
...
# THE REGION, DESCRIBED. Only what the mask actually covers: the guard's harness
# strap crossing his chest, his cream shirt, the brown cuff, in this plate's own
# flat-cel dialect. No figure noun of any kind -- there is no person IN this
# region, only clothing on one.
POSITIVE = (
    "brown leather harness strap crossing a cream linen shirt, brown cuff, "
    "white sash, flat cel shading, clean ink linework, soft cinematic lighting, "
    "anime key art, detailed, masterpiece, best quality, very aesthetic\n")

# THE PARENT'S NEGATIVE, KEPT WHOLE, PLUS THE TWO NOUNS BOTH SAMPLES DREW. The
# additions are not speculative: `goblin`, `green skin` and `pointed ears` name
# what fistcopy drew; `hand`, `fist`, `fingers` name what eraseonly drew; and
# `buttons, placket` names the shirt detail fistcopy invented where the strap
# had been.
NEGATIVE = (
...
DRY_NOTE = (
    ...
    "rung changes the PROMPT and nothing else: same init sha, same mask sha, "
    "same strength 0.99, same 40 steps, same cfg 7.5. The mask is the guard's "
    "original left fist grown 10, 10020 px, largest component 9956 px in a "
    "102x118 box at the CHEST. WHAT TO CHECK ON THE DRY PNG: one compact white "
    "blob at roughly x 555..660 y 528..635 with the strap running through it, "
    "NO white at the board's top edge near y~670, and no white on the guard's "
    "FACE (x 531..601 y 355..420) or the goblin (x 60..260 y 430..1140). If it "
    "differs from ep2-b08-eraseonly-0820's dry mask at all, something other "
    "than the prompt moved and this job must stop here at $0.")

fresh = {
    "why": (
        "TWO SAMPLES, TWO GOBLINS. fistcopy's 18408 px mask drew a whole goblin "
        "head; eraseonly's 10020 px mask -- 1.8x smaller by area -- drew a green "
        "goblin FIST in place of the guard's hand. Shrinking the region changed "
        "the size of the noun and not the noun. I pre-committed the next step as "
        "strength 0.99 -> 0.70 and I am NOT taking it, because the second sample "
        "distinguished what the first could not: what arrives is not any noun, "
        "it is specifically the one the prompt names twice -- 'the small goblin "
        "man' and 'green skin' -- at a region that is entirely GUARD. The "
        "unchanged wording was the right discipline for testing the mask and it "
        "is the wrong prompt for this pass. This rung describes what is actually "
        "in the region and negatives the two nouns both samples drew."),
    "consumer": (
        "BEAT 08's PLATE DECISION, third and last cheap attempt on this route. "
        "The question is unchanged -- can the sampler delete the guard's fist "
        "from real strap pixels -- and this is the last lever that costs nothing "
        "to pull. If a region-appropriate prompt still returns a hand or a "
        "goblin, the pre-committed step is strength 0.70, and after that the "
        "honest conclusion is that a tool with NO spatial conditioning cannot be "
        "asked to erase a limb, and beat 08's grip goes back to the txt2img "
        "route with two measured samples saying why."),
    "success": (
        "ONE 832x1216 PNG plus its sidecar, published into courier-box, off the "
        "same asserted init sha as the parent. The bar is below and it is the "
        "parent's bar with H5 sharpened by what the parent actually drew. NOT a "
        "pick and NOT a plate_ack."),
    "owner": ("beat 08 arm-route lane, 2026-08-20 -- derived by "
              "pipeline/derive_b08_nogoblin_0820.py, init UNCHANGED from "
              "ep2-b08-eraseonly-0820 (pipeline/beat08_grip_copy.py "
              "--variant eraseonly)"),
}
...
overrides = {
    ...
    "key:script_authority": (
        ...
        "digit is invented."),
    # fetch_init.py, the init and the mask are all UNCHANGED from the parent:
    # this rung moves the PROMPT and nothing else, so they inherit.
...
extra = {
    ...
    "what_the_two_parent_runs_established": (
...
child = derive_spec.derive(
    src="pipeline/jobs/ep2-b08-eraseonly-0820.yaml",
    new_id=JOB,
    fresh=fresh,
    overrides=overrides,
    retoken=[("b08eraseonly-0820", "b08nogoblin-0820"),
             ("b08-eraseonly", "b08-nogoblin"),
             ("s20260821", "s%d" % SEED)],
    extra=extra,
    by="pipeline/derive_b08_nogoblin_0820.py")
