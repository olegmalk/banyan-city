#!/usr/bin/env python3
r"""PASS ONE, RELIT: the seated plate the LoRA will repaint, in the canon's light.

THE TWO-PASS WORKS AND THIS IS THE ONE THING WRONG WITH IT.
`twopass-b13seat-s065` came back SEATED -- knees up, forearms on the knees,
hands clasped, beat 13's own stance -- and GOBLIN: dome, pointed near-horizontal
ears, narrow almond eye, tiny dark pupil, smooth face, all correct at 1:1
against taste/refs/goblin-canon-founder-0821.png. Pose and identity in one frame
for the first time in seven rounds.

His SKIN came back mauve, and there is a cowlick at the crown. Both are pass
one's, not pass two's, and the mechanism says so: `goblin-i2i-route-0822.md`
closed the i2i route with the finding that it RE-LIGHTS AND DOES NOT RE-GROUND.
Pass two inherits the init's light by construction. Pass one's prompt named no
light key at all, the sampler chose a purple twilight, and pass two faithfully
kept it -- which is the route behaving exactly as measured, not a defect in it.

SO THE FIX IS IN PASS ONE'S WORDING AND NOWHERE ELSE. Same skeleton, same net,
same scale, same seed, no LoRA -- the cell that already drew a correct seated
figure -- with the daylight key the founder's own canon frame is lit by added to
the prompt. Turning a knob in pass two would have been treating an inherited
property as a local one.

  python3 pipeline/lora/emit_twopass_p1_jerry_0822.py            # dry
  python3 pipeline/lora/emit_twopass_p1_jerry_0822.py --write
"""
...
OUT = "pipeline/lora/twopass-p1day-jerry-0822.yaml"
...
NOTES = {
    "tp1day-b13seat": (
        "PASS ONE, RELIT. No LoRA -- this plate exists to carry a POSE and a "
        "LIGHT, and pass two puts bnyjerry into it. Identical to round six's "
        "control cell, which drew a correct h240seat stance, except that the "
        "prompt now names the flat overcast daylight the founder's canon frame "
        "is lit by. The i2i route re-lights and does not re-ground, so pass "
        "two inherits this light -- which is why the mauve skin is fixed here."),
}
...
def main() -> int:
    ...
    spec = {
        ...
        "priority": 65, "max_attempts": 1, "est_minutes": 4,
