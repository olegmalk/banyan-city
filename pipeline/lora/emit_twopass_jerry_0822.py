#!/usr/bin/env python3
r"""THE TWO-PASS: let the net pose a stranger, then let the LoRA make him himself.

WHY THIS EXISTS, AND IT IS BUILT ENTIRELY OUT OF PARTS THAT ARE ALREADY MEASURED.
Round seven closed the one-pass route on the founder's own age dial, and the two
curves do NOT cross:

  weight 0.80  goblin, standing, hands clasped   -- identity yes, pose no
  weight 0.65  goblin, standing, hands clasped   -- identity yes, pose no
  weight 0.50  a dark-haired human boy, standing -- identity GONE, pose still no
  weight 0.35  a dark-haired human boy, standing -- identity GONE, pose still no
  weight 0.80 at hint scale 1.4                  -- identity yes, pose no

His face leaves between 0.65 and 0.50 and the seated legs have still not arrived
by 0.35. Every cell adopted the hint's UPPER body -- clasped hands, forearms in,
a forward lean -- and not one adopted the leg fold. The lock is a LOWER-BODY
prior and the cause was pre-registered before the weights existed: 21 of 21
training frames are standing. No knob reaches that, which is why round seven is
the last knob round rather than the first of five.

BUT ROUND SIX'S CONTROL CELL IS A CORRECTLY SEATED FIGURE, and it cost nothing.
With no LoRA in the pass, beat 13's own skeleton drew the h240seat stance --
knees up, forearms on the knees, hands clasped. The pose exists. It is simply
wearing the wrong person. So the two things this tree needs are each available;
they just cannot be asked of one denoising pass.

SO ASK THEM OF TWO, AND THE SECOND PASS IS A ROUTE THAT IS ALREADY CLOSED WITH A
FINDING RATHER THAN A FAILURE. `goblin-i2i-route-0822.md` closed img2img-from-
canon as a POSING route on a mechanism -- at low strength only the last s*N steps
run, global structure is decided in the high-noise steps the pass never enters,
so a skeleton at scale 1.0 moved grass and light by 15.05 mean abs and did not
bend a knee. That is a failure as a posing route and it is exactly the property
wanted here: A PASS THAT CHANGES APPEARANCE AND CANNOT CHANGE STRUCTURE. The
seated composition is now in the INIT, where the LoRA cannot argue with it, and
the LoRA gets the only job it is good at.

  pass 1 (DONE, b2r6-b13seat-base)  no LoRA + skeleton at 1.0 -> the pose
  pass 2 (THIS JOB)                 that plate as the init, all-white mask,
                                    LoRA at 0.8, same skeleton still attached,
                                    strength BELOW 1.0 so structure survives

TWO STRENGTHS, BECAUSE THE ONE NUMBER THAT MATTERS HERE HAS NEVER BEEN MEASURED
WITH A LoRA IN THE PASS. 0.50 and 0.65 bracket the band the i2i route measured
as re-lighting without re-grounding. Too low and he stays a human boy; too high
and the seated legs go the way they went in round seven. The bar is unchanged
and it is both halves: HE SITS, and he is himself at 1:1 against
taste/refs/goblin-canon-founder-0821.png.

  python3 pipeline/lora/emit_twopass_jerry_0822.py            # dry
  python3 pipeline/lora/emit_twopass_jerry_0822.py --write
"""
...
JOB = "lora-jerry-v2-twopass-0822"
...
OUT = "pipeline/lora/twopass-jerry-0822.yaml"
...
INIT_BOX = (r"C:\banyan-farm\courier-box\farm-out\lora-jerry-v2-b2r6-0822"
            r"\b2r6-b13seat-base.png")
INIT_SHA = "5f7d41a616edf80744d9961a573dafc59a003944272b65a535c01aecd415acef"
...
NOTES = {
    "twopass-b13seat-s050": (
        "THE TWO-PASS at strength 0.50. Pass one's seated plate as the init, "
        "all-white mask, bnyjerry at 0.8, beat 13's skeleton still attached at "
        "1.0. The i2i route's own mechanism is the reason this can work: at low "
        "strength the high-noise steps that decide structure never run, so the "
        "seat is not up for negotiation and the LoRA only repaints."),
    "twopass-b13seat-s065": (
        "THE TWO-PASS at strength 0.65. The upper bracket. Too high and the "
        "seated legs go the way they went in round seven's ladder; this cell "
        "says where that edge is."),
}
...
def steps():
    ...
    for s in ("0.50", "0.65"):
        out.append(cell("twopass-b13seat-s%s" % s.replace(".", ""), "0.8",
                        strength=s))
...
def main() -> int:
    ...
    spec = {
        ...
        "priority": 64, "max_attempts": 1, "est_minutes": 5,
