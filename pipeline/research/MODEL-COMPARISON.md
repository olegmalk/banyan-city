...
| **TI2V-5B + SageAttention 2.2.0, TRITON kernel on sm_120** — `DIFFUSERS_ATTN_BACKEND=_sage_qk_int8_pv_fp16_triton`, isolated venv `C:\banyan-bench\sage-venv` (sageattention + triton-windows only; torch/diffusers/transformers read-only from the production tree via a `.pth`, so `C:\banyan-video\venv` was never written to — verified, it imports neither package) | T1-A/B's recipe **held byte-for-byte**: 704x1280, 61f @24fps = 2.542s, 14 steps, guidance 5.0, UniPC `flow_shift` 5.0, same prompt (243 chars) and negative (618 chars, recomposed from the baseline sidecar and asserted equal), same conditioning still (sha256 `004ecf2d…`), seed 20260732, beat 1, `model_cpu_offload`. **The only difference from its control is the environment variable** | production | **5.86 s/it settled (steps 8-14) against the control's 8.57 — −31.6%, a 1.46x kernel.** Denoise loop **89s vs 126s** (−29.4%); sample **137s vs 179s**; wall 160s vs 272s. The ~48s outside the loop (text encode, VAE decode, h264) is unchanged, as it must be — sage touches only the transformer | **0.0186 s(video)/s(wall) @b1** = 53.9s wall per 1s video, against the control's 0.0142 (70.4s) | **14.4GB torch / 1.8GB device of 26GB** — LABEL: `model_cpu_offload`. **Identical to the control to the tenth of a GB: the speedup costs no VRAM** | `no data` — the host sampler does not run on the `probe_beat` path | 1/card | SageAttention **Apache-2.0** — a real LICENSE file in both `thu-ml/SageAttention` and the `woct0rdho` fork, so the scheduled row's "pending confirmation" is **resolved**; `triton-windows` **MIT** | **MEASURED-BY-US 2026-08-07**, $0, rc=0. **mobcat40's ~35% is very nearly right, and it is now ours rather than his: 31.6% per step on our exact card.** The drift is real and is NOT free — **rms 4.293/255, PSNR 35.48 dB** against a 1.006 crf23 and 1.704 bitrate-matched control (4.27x the encode-noise floor), and **fully attributable**, because the run-to-run floor here is *exactly zero* (see the control row). For scale it is ~⅓ of what the fp8 cast costs (11.93) and of batch-2 drift (10.35). **0/60 frozen frames; colour unmoved** (ΔR −0.031, ΔG −0.073, ΔB −0.050; consecutive-frame MSE mean 89.25 → 91.70, i.e. marginally *more* inter-frame motion). **NOT SCREENED — the founder has not seen this clip, and adoption is his call (R4), not this table's** | `bench-platform/t4-sage-triton.mp4.meta.yaml`, `bench-platform/t4-sage-20260807.log`, `bench-platform/t4-sage-fidelity-20260807.txt` |
| same — **the NATIVE control, and the run-to-run determinism floor** | the sage row's command **verbatim**, same isolated venv and same harness; `DIFFUSERS_ATTN_BACKEND=native`. Run twice | production | **8.57 s/it settled, both runs** (loop 126s and 127s; sample 179s and 172s, wall 272s and 194s) — and 8.57 reproduces the 2026-08-04 archived baseline's **8.59** | 0.0142 s(video)/s(wall) @b1 = 70.4s per 1s video | 14.4GB torch / 1.8GB device of 26GB — `model_cpu_offload`, matching the archived baseline exactly | `no data` | 1/card | Apache-2.0 | **MEASURED-BY-US 2026-08-07**, $0. **The two runs are BYTE-IDENTICAL — sha256 `6cb0c84d…`, 336883 bytes both times. The TI2V-5B path is bit-deterministic run to run on this box**, which was assumed and had never been shown (the zero-drift control on record is LTX's). That is what licenses the sage row's drift figure. **Note the totals, per §3 rule 8: the same byte-identical recipe scored 179s and 172s**, so quote per-step. These clips are NOT byte-comparable to `bench-T1T2T3/T1-shift5.0.mp4` — that one was encoded at crf 16 (1216053 bytes) and `probe_beat` writes `export_to_video`'s default (336883); the denoise reproduced, the h264 settings did not | `bench-platform/t4-native-control.mp4.meta.yaml`, `bench-platform/t4-native-repeat.mp4.meta.yaml`, `bench-platform/t4-sage-fidelity-20260807.txt` |
...
| TI2V-5B + **SageAttention 2.2.0 sm_120** | 704x1280, 14 steps, separate venv (wheel pins the torch line) | — | — (**~160s/beat CLAIMED-BY-mobcat40**, ~35%, measured on an RTX 5090 Laptop 24GB) | — | no data | no data | 1/card | tooling: Apache-2.0 pending confirmation | **SUPERSEDED — measured 2026-08-07**, see the two T4 rows above. The claim held: ~35% claimed, **31.6% per step measured**. Two things in this row's own text were wrong, though, and both were wrong in the direction of "harder than it is" — **the wheel does not pin the torch line** (`cp310-abi3` + `torch2.10.0andhigher`, libtorch stable ABI, installed on our released torch 2.11.0+cu128 and Python 3.12.10), and the licence needed no confirming, it ships a real Apache-2.0 LICENSE | `ACTION-PLAN.md §1 T4`, `DECISION.md §2` |
