#!/usr/bin/env python3
r"""node 001 beat 06 — ROUND 6, TOO BLUE. His two options, one sample each. 2026-08-10.

HE REJECTED EVERY CANDIDATE THIS BEAT HAS EVER HAD AND THEN TOLD US THE SHOT.
Founder (R4), 2026-08-10 ~15:00 local, verbatim:

    "none of the beat 6 images will work, you either show the ground as having
    tall grass or dont show the ground at all and show the sky, right now its
    showing the ground as some flat place, or showing cities for some reason"

That is a reject of r3, r4, r5 and the `fix` set, plus a composition with
exactly two acceptable readings and two named defects. This script renders ONE
CANDIDATE SET PER READING and picks between them not at all — the choice is his
and showing him both is the whole point.
...
Usage:
    python render_b06r6.py --root C:\banyan-farm\banyan-city --arm A --measure
    python render_b06r6.py --root C:\banyan-farm\banyan-city --arm A --dry
    python render_b06r6.py --root C:\banyan-farm\banyan-city --arm A --out <dir>
"""
import argparse
import hashlib
import sys
...
def sidecar(png: Path, *, arm: dict, arm_key: str, seed: int, pos: str, neg: str,
    ...
    lines = ["# Still provenance (7.2), written AT RENDER TIME by render_b06r6.py",
    ...
    body = ["platform: local-gpu (rtx5090)",
            ...
            "arm_is_not_a_pick: >-",
            block("Round 6 has TWO arms because the founder gave two acceptable "
            ...
            f"urban_negatives_added: {arm['extra_neg']}",
            "urban_negatives_tier: explicit (last in fit_negative's drop order)",
...
NOTE_TEMPLATE = (
    "round 6, ARM {arm} — {label}. Every beat-6 candidate this tree has ever "
    "drawn was rejected by the founder on 2026-08-10, with a composition given "
    "in the same sentence: \"you either show the ground as having tall grass or "
    "dont show the ground at all and show the sky, right now its showing the "
    "ground as some flat place, or showing cities for some reason\". Two "
    "readings, two arms, one candidate set each, and the steward picks between "
    "them nowhere. {why} THE CITIES WERE NEVER FORBIDDEN: every negative this "
    "beat has sent, read back from its own sidecars across r3, r4, r5 and the "
    "fix set, is free of city, cityscape, building, skyline, skyscraper, "
    "industrial, town and architecture — and r5's negative had 26 spare tokens "
    "(51 of 77 on real CLIP), so this was never a trim. The positive was asking "
    "for them: r5's `scenery` is the Danbooru umbrella whose children include "
    "`cityscape` (62,273 posts to 19,945), it is new in r5, and r5 is where the "
    "city and the industrial skyline appear. Both arms delete the tag and negate "
    "the block. WHAT IS HELD FIXED: `no humans` at the head of the POSITIVE with "
    "no person noun in the negative — r5's one confirmed win, zero people in 4 "
    "of 4, and not mentioned in his verdict. Seeds are r5's own four. shots.md "
    "NOT edited; sd_prompt.py NOT modified. Nothing here is a pick, a promotion, "
    "a publication or a spend.")
...
def main() -> int:
    ...
    arm_key = a.arm
    arm = ARMS[arm_key]
    task = a.task or f"001-b06-r6{arm_key.lower()}-{int(time.time())}"
