...
BEATS = {
    ...
    6: dict(
        ...
        candidates=[
            dict(file=f"{POSTER_URL}/06-the-clipboard-PLATE-pose-0822.jpg", still=True,
                 label="a PLATE — a different method entirely, and it puts him in the field", tag="warn",
    ...
    11: dict(
        ...
        wins=[],
        note="STATUS, 2026-08-22: nothing new was rendered for this beat tonight and nothing is queued for it. It is the only beat in the episode with no goblin and no open plate question, so it did not compete for the card against beats that have never had a candidate. Its named next step is unchanged and is cheap: an earlier pass on it was withdrawn over \u201ctotal identity collapse away from the plate\u201d across frames 16 to 21, and that window was later explained by a compression flag on the conditioning image rather than by the plate \u2014 so the first move is a re-run at the corrected setting, not a new plate. It has not been fired because the card has been full of beats with nothing at all under them.",
