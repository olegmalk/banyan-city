    ...
    6: dict(
        ...
        candidates=[
            dict(file=f"{POSTER_URL}/06-the-clipboard-PLATE-boardnat-r2-0822.jpg", still=True,
                 label="a PLATE — HE IS HOLDING A BARK BOARD, the right size, and it looks like bark", tag="warn",
                 verdict="This is the beat's fault answered. A grown man in a field holding a large flat slab of bark in both hands at chest height, as wide as his shoulders, head down over it — and the slab now has WOOD IN IT: a warm grain running across the face, a torn irregular edge, a bark colour rather than a flat brown fill. It has not moved, it has not narrowed, it is not glowing, and he is unchanged behind it. WHAT IS LEFT, and it is small: a soft crease down the middle makes it read for a second like an open book rather than one piece of bark. Not in the cut yet — this is a still, and the motion render off it is the next thing.",
