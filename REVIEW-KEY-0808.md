...
NEXT four seeds of the beat's own series — **columns on the `beat06-r3` sheet are
NOT controlled pairs**. r4 reuses r3's own four seeds, so **columns on the
`beat06-r4` sheet ARE controlled pairs**. There is no `r2` for this beat (the
round-2 tag belongs to beats 3 and 12; same label logic as beat 15's jump to r3).
...
| **`b06-r3-s0`** | `06-too-blue-r3-s0.png` | 20264725 | `beat06-r3`, `beat06-r4` |
...
Beat 14 — roots in soil, warm raking light. **r4 is the newest set** (drawn
2026-08-09); r3 was rejected on all four the same day — *"for beat 14 they are
all too short"* — on top of *"all too small"* on r1 (2026-08-08). r3 answered the
r1 verdict by filling the frame; r4 answers "short" by moving HEIGHT rather than
area, taking `b15-r3-s1`'s own subject clause — the one frame on this tree he has
passed — onto this beat's framing, plus an apex near the top of the frame and a
low soil line so the earth stops taking the picture's share. His lens note is
kept verbatim.
...
| **`b14-r3-s0`** | `14-worth-staying-in-r3-s0.png` | 20260733 | `beat14-r3`, `beat14-r4` |
| **`b14-r3-s1`** | `14-worth-staying-in-r3-s1.png` | 20261733 | `beat14-r3`, `beat14-r4` |
| **`b14-r3-s2`** | `14-worth-staying-in-r3-s2.png` | 20262733 | `beat14-r3`, `beat14-r4` |
| **`b14-r3-s3`** | `14-worth-staying-in-r3-s3.png` | 20263733 | `beat14-r3`, `beat14-r4` |
| **`b14-r4-s0`** | `14-worth-staying-in-r4-s0.png` | 20260733 | `beat14-r4` |
| **`b14-r4-s1`** | `14-worth-staying-in-r4-s1.png` | 20261733 | `beat14-r4` |
| **`b14-r4-s2`** | `14-worth-staying-in-r4-s2.png` | 20262733 | `beat14-r4` |
| **`b14-r4-s3`** | `14-worth-staying-in-r4-s3.png` | 20263733 | `beat14-r4` |

Same seeds down each column, in all three rounds — controlled pairs throughout.

**The steward's provisional pick is `b14-r4-s0`, and it is disclosed here in
words because it is NOT marked on the sheet.** Ledger record
`ep1-b14-r4-provisional`, confidence 0.40 — deliberately low, and the reasons are
recorded against the pick rather than hidden. s0 and s3 are the same frame but
for the ground: s0 has a soil line with the stem entering the earth, s3 has no
ground at all, and the beat's own script line is *"Low at the base of the trunk:
roots gripping soil"*, so A3 breaks a 26-to-25 tie toward s0. **s3 is the flip if
"short" is still the answer** — it is the taller frame — and s1 is taller still
but grows six leaves on a woody stem, a different plant from the two-cotyledon
sapling every other beat draws. s2 draws TWO sprouts, so the recurring subject is
ambiguous in a shot that is supposed to be him.

**A fault in all four, recorded before you find it: there are no roots.** The
prompt asks for *"pale roots gripping damp brown soil"* and not one frame draws
one. If the set is rejected, this is the likelier axis after height.
...
| `LABELED-beat14-r3.png` | beat 14, r1 + r3, 8 frames, same seed per column | superseded by `beat14-r4` (all four r3 rejected 2026-08-09) |
